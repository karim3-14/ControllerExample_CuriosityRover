...
%     xf(4)=0;
% else
    %X trajectories phase 2
    xf(1) = 0*t;
    xf(3) = 0*t;
    
 
    %Z trajectories phase 2
%     xf(2) = 200-1.9*(t-60);
%     xf(4) = 10*(1-exp(-(t-60)/90)) + 3;

xf(2) = Zp2C(1) + Zp2C(2).*t + Zp2C(3).*t.^2 + Zp2C(4).*t.^3;
xf(4) = 0+Zp2C(2) + 2*Zp2C(3).*t + 3*Zp2C(4).*t.^2;
    

% % Q trajectories phase 2
% xf(5) = 0;
% xf(7) = 0;

% L trajectories phase 2
xf(5) = 15;
xf(6) = 0;

a = -Kp*([x(1);x(3);x(5)]-[(xf(1));(xf(2));xf(5)]) - ...
    Kd*([x(2);x(4);x(6)]-[xf(3);xf(4);xf(6)]);
...
Fx = 0.5*(Cd*m1*x(2)^2)+a(1)*(m1+m2-Cd*m1*x(1))+x(5)*dq*m2*sin(q);
Fz = 3.7*m1+3.7*m2 + a(2)*(m1+m2-Cd*m1) + x(5)*dq*m2*cos(q);
% Tau = x(5)*(x(5)*a(3) + 3.7*m2*sin(q) - m2*x(3)*cos(q) - ...
%     m2*x(1)*sin(q) + dq*m2*x(1)*cos(q) - dq*m2*x(3)*cos(q));
Fl = m2*(a(3)-3.7*cos(5) - x(5)*dq^2 + dq*x(3)*cos(q) + ...
    dq*x(1)*sin(q));
...
dX(2) = (Fx - (0.5*Cd*m1*x(2)^2) - ...
    x(5)*dq*m2*sin(q))/(m1 + m2 - Cd*m1*x(1));
dX(3) = x(4);
dX(4) = (Fz - 3.7*(m1 +m2) - x(5)*dq*cos(q)*m2)/(m1+m2-Cd*m1); 
dx(5) = x(6);
dx(6) = ((Fl/m2) +3.7*cos(q) + x(5)*dq^2 - dq*x(3)*cos(q) ...
    -dq*x(1)*sin(q));
% dx(5) = 1;
% dx(6) = 0;
end